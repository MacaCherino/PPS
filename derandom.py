...
pn = [
  ...
  0x19, 0x51, 0x5f, 0x9f, 0x05, 0x08, 0x78, 0xc4, 0x4a, 0x66, 0xf5, 0x58];

# funcion para eliminar palabra de sincronizacion
def extract_SW(data):
    frame_out=data[SYNCWORDSIZE:]   # se toman los bits a partir del bit 32
        
    return frame_out

# funcion para agrupar bits en bytes
def group_bytes(frame):
    frame_out=np.zeros(FRAMESIZE-4,np.uint8)    # se define arreglo para almacenar trama en bytes
    for i in range (0,FRAMESIZE-4):             # se recorren todos los elementos de la trama de salida
        byte_out=0                              # se inicializa byte de salida en 0
        for j in range (0,8):                   # se recorren 8 bits consecutivos de cada trama
            byte_out|=int(frame[i*8+j])<<(7-j)  # se desplaza el bit en la posicion correspondiente para conformar el byte de salida
        frame_out[i]=byte_out                   # se almacena el byte de salida en la posicion correspondiente
        
    return frame_out

def derand():

    nrz_out = np.fromfile(open('files/test3/nrz_out.bin',"rb"), dtype=np.uint8)         # se extraen los 8192 bits que conforman la trama a la salida de la decodifiacion diferencial
    size = len(nrz_out)//FRAMEBITS                      # se define la cantidad de tramas
    # se reacomodan los datos extraidos por tramas
    frames = np.zeros((FRAMEBITS,size),np.uint8)        
    for i in range(0,FRAMEBITS):                        
        frames[i,:]=nrz_out[i*size:(i+1)*size]
    
    derand_out = np.zeros((FRAMESIZE-4,size),np.uint8)  # se define arreglo para almacenar tramas desaleatorizadas
    for j in range(0,size):                             # se recorren todas las tramas
        frame = frames[:,j]                             # se toma una trama
        decoded_noSW = extract_SW(frame)                # se elimina la palabra de sincronizacion
        data_bytes = group_bytes(decoded_noSW)          # se agrupan los bits por bytes
        decoded_data = np.zeros(FRAMESIZE-4,np.uint8)   # se define arreglo para almacenar trama desaleatorizada
        for k in range (0,FRAMESIZE-4):                 # se recorren los bytes de la trama aleatorizada
            decoded_data[k] = data_bytes[k]^pn[k]       # se aplica una operacion XOR entre el byte y el elemento del polinomio generador correspondiente
        derand_out[:,j] = decoded_data                  # se almacena trama desaleatorizada
...
if __name__ == "__main__":
    derand_frame = main()
    # se almacenan las tramas desaleatorizadas en un archivo.bin
    derand_frame.tofile('files/test3/derand_out.bin')
